        ...
        for y in range(0, celX):
            for x in range(0, celY):
                ...
                if not pause: 
                    ...
                    if gameState[x,y] == 1 and neighbor != 3:
                        rand = randint(1,8)
                        move = point(rand, gameState, x, y)
                        if (move == 0):
                            gameState[x,y] = 0
                            walk = mov(rand, copyState, x, y)
                            walk = 1
...
def point(rand, gameState, x, y):
    switcher = {
        1: gameState[(x-1) % celX, (y-1) % celY],
        2: gameState[(x) % celX, (y-1) % celY],
        3: gameState[(x+1) % celX, (y-1) % celY],
        4: gameState[(x-1) % celX, (y) % celY],
        5: gameState[(x+1) % celX, (y) % celY],
        6: gameState[(x-1) % celX, (y+1) % celY],
        7: gameState[(x) % celX, (y+1) % celY],
        8: gameState[(x+1) % celX, (y+1) % celY]
        }
    return switcher.get(rand, gameState[(x-1) % celX, (y-1) % celY])

def mov(rand, copyState, x, y):
    switcher = {
        1: copyState[(x-1) % celX, (y-1) % celY],
        2: copyState[(x) % celX, (y-1) % celY],
        3: copyState[(x+1) % celX, (y-1) % celY],
        4: copyState[(x-1) % celX, (y) % celY],
        5: copyState[(x+1) % celX, (y) % celY],
        6: copyState[(x-1) % celX, (y+1) % celY],
        7: copyState[(x) % celX, (y+1) % celY],
        8: copyState[(x+1) % celX, (y+1) % celY]
        }
    return switcher.get(rand, copyState[(x-1) % celX, (y-1) % celY])
